...
type View = 'dashboard' | 'clients' | 'invoices' | 'quotes' | 'cash' | 'expenses' | 'imports' | 'settings';
...
function ClientEditor({ initial, onClose, onSave }: { initial: Client; onClose: () => void; onSave: (c: Client) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal form-modal" onMouseDown={e => e.stopPropagation()}>
    ...
    <form onSubmit={e => { e.preventDefault(); if (value.name.trim()) onSave({ ...value, name: value.name.trim() }); }}>
      <div className="form-grid"><label className="wide">Dénomination sociale *<input value={value.name} onChange={e => update('name', e.target.value)} required placeholder="Nom officiel du client" /></label>
        ...
        {value.billingModel === 'monthly' && <><label>Début du contrat / suivi<input type="date" value={value.monthlyStart} onChange={e => update('monthlyStart', e.target.value)} /></label><label>Fin du contrat (si connue)<input type="date" value={value.monthlyEnd} onChange={e => update('monthlyEnd', e.target.value)} /></label><label>Montant mensuel HT (si fixe)<input type="number" min="0" step="0.01" value={value.monthlyAmountHt ?? ''} onChange={e => update('monthlyAmountHt', e.target.value === '' ? null : Number(e.target.value))} /></label><label>Jour habituel de facturation<input type="number" min="1" max="31" value={value.billingDay ?? ''} onChange={e => update('billingDay', e.target.value === '' ? null : Number(e.target.value))} /></label></>}
...
function DocumentEditor({ initial, clients, documents, onClose, onSave, onDelete }: { initial: Document; clients: Client[]; documents: Document[]; onClose: () => void; onSave: (d: Document) => void; onDelete: (d: Document) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal document-modal" onMouseDown={e => e.stopPropagation()}>
    ...
    <form onSubmit={submit}>
      ...
      <div className="form-grid three"><label>Client *<select value={value.clientId} onChange={e => update('clientId', e.target.value)} required><option value="">Choisir un client</option>{clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
        <label>Référence<input value={value.reference} onChange={e => update('reference', e.target.value)} placeholder={value.kind === 'invoice' ? 'FC001' : 'DV001'} /><small>Vide = brouillon. Vérifier la série 2026 avant d’attribuer un numéro.</small></label>
        ...
        <label>Catégorie<select value={value.category} onChange={e => update('category', e.target.value as Document['category'])}><option value="oneoff">Ponctuelle</option><option value="monthly">Mensuelle</option></select></label>
        ...
        {value.category === 'monthly' && <><label>Période du<input type="date" value={value.periodStart} onChange={e => update('periodStart', e.target.value)} /></label><label>Au<input type="date" value={value.periodEnd} onChange={e => update('periodEnd', e.target.value)} /></label></>}
...
export default function App() {
  ...
  const saveExpense = (value: Expense) => setDb(prev => ({ ...prev, expenses: prev.expenses.some(item => item.id === value.id) ? prev.expenses.map(item => item.id === value.id ? value : item) : [...prev.expenses, value] }));
  const deleteExpense = (value: Expense) => { if (confirm(`Supprimer « ${value.label} » ?`)) setDb(prev => ({ ...prev, expenses: prev.expenses.filter(item => item.id !== value.id) })); };
